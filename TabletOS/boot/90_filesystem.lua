...
fs.setPermissionFile = function(file,permissions)
  if fs.exists(file) then
    local name = fs.name(file)
    local path = fs.path(file)
    local permissionFile = path + "." + name
    local vFile = "return {"
    vFile = vFile .. "execute = " .. tostring(permissions.execute) .. ","
    vFile = vFile .. "read = " .. tostring(permissions.read) .. ","
    vFile = vFile .. "write = " .. tostring(permissions.write) .. "}"
    fs.remove(permissionFile)
    local file = io.open(permissionFile,"w") 
    file:write(vFile)
    file:close()
  end
end

fs.getPermissionFile = function(file)
  if fs.exists(file) then
    local name = fs.name(file)
    local path = fs.path(file)
    local permissionFile = path + "." + name
    return dofile(permissionFile)
  end
end
...
--[[_G.permissions = {
  ["/init.lua"] = {type="file",execute=true,read=false,write=true},
  ["/autorun.lua"] = {type="file",execute=false,read=false,write=true},
  ["/lib/filesystem.lua"] = {type="file",execute=false,read=false,write=true},
  ["/boot/"] = {type="dir",execute=true,read=false,write=true},
  ["/bin/"] = {type="dir",execute=false,read=false,write=true},
}]]
