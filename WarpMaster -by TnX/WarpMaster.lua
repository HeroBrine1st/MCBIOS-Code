...
local updaterPaths = {
	url = {
			"http://5.128.173.156:8080/WarpMaster.lua",
			"http://5.128.173.156:8080/Icon.pic",
			"http://5.128.173.156:8080/WarpMasterIcon.pic",
			"http://5.128.173.156:8080/libwarp.lua",
			"http://5.128.173.156:8080/Russian.txt"		
			},
	path = {
			"MineOS/Applications/WarpMaster.app/WarpMaster.lua",
...
--есть информация, что точки могут быть неактуальны. Если знаете актуальные данные, пишите мне.
...
--к сожалению, как оказалось, управляющего метода у генератора воздуха нет. когда добавят, можно будет быстро допилить.
...
function WGUI.DrawExitScreen()
	ecs.emptyWindow(30,15,40,20,"Warp Navigation Master")
	ecs.colorText(40, 24, colors.text, "Application Closing")
	ecs.colorText(40, 25, colors.text, "Thank you for")
	ecs.colorText(40, 26, colors.text, "using WarpMaster!")
end
...
function WGUI.DrawNewNavPointWindow(x,y,z)
	...
	{"Selector", 0x262626, 0x880000, "earth", "space", "hyper", "planet"},
	{"EmptyLine"},
...
function WGUI.DrawViewPointInfoDialog(pointIndex)
	...
	{"Button", {0x57A64E, 0xffffff, okText},{0x57A64E, 0xffffff, setText},{0x57A64E, 0xffffff, autopilotText},{0xCC4C4C, 0xffffff, removeText}}
	)
...
--поправить баг с нерабочей проверкой безопасности
